...
struct TRANS{
    string num;
    string order;
    TRANS(string num,string order):num(num),order(order){}
    TRANS(){}
    void length(){
        while(num.length()<4){
            num="0"+num;
...
TRANS D(TRANS &t){
    ...
    TRANS rt(to_string(a),t.order+"D");
    rt.length();
    return rt;
}
TRANS S(TRANS &t){
    ...
    TRANS rt(to_string(a),t.order+"S");
    rt.length();
    return rt;
}
TRANS L(TRANS &t){
...
TRANS R(TRANS &t){
    string num=t.num;
    string rt_num="";
    rt_num+=num[num.length()-1];
    for(int i=0;i<num.length()-1;i++){
        rt_num+=num[i];
    }
    TRANS rt(rt_num,t.order+"R");
    rt.length();
    return rt;
}
bool visited[10000];
void bfs(int start,int end){
    ...
    while(!q.empty()){
        ...
        for(int i=0;i<4;i++){
            ...
            if(visited[stoi(nt.num)]) continue;
            q.push(nt);
        }
        idx++;
    }
