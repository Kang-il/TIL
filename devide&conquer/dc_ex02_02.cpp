...
void solution(){
    ...
    vector<int> ans(M);
    for(int i=0;i<M;i++){
        // auto -- vecotr<int>::iterator
        auto lower = lower_bound(myCard.begin(),myCard.end(),compCard[i]);
        auto upper = upper_bound(myCard.begin(),myCard.end(),compCard[i]);
        cout<< upper - lower <<' ';
    }
    cout<<'\n';
}
